##Set Directory
setwd("I:/Articles/Motivation - DIF_DTF")
...
## Load data sets
dataw <- read.csv("white.csv", header = TRUE, sep=",")
white <-(dataw[,1:6])
datao <- read.csv("other_race.csv", header = TRUE, sep=",")
other <-(datao[,1:6])
WhiteN <- 2220
OtherN = 1001
group <- c(rep('Reference', WhiteN), rep('Focal', OtherN))
dat <- rbind(white, other)

############# Step 1: Testing assumptions ############

## Graded Response Model Fit
ref.model <- mirt(white, model = 1, itemtype = "graded", SE=TRUE, verbose = FALSE)
M2(ref.model, type = "C2")
foc.model <- mirt(other, model = 1, itemtype = "graded", SE=TRUE, verbose = FALSE)
M2(foc.model, type = "C2")

## Item Fit
(white.fit <- itemfit(ref.model))
(other.fit <- itemfit(foc.model))
...
anc.items.names <- itemnames[c(4)] #Top 5 a-parms from Step 4 as anchors
test.items <- c(1,2,3,5,6) #DIF items from Step 4
...
(dif.anchor <- DIF(model_anchor, c('a1','d1','d2','d3','d4'), items2test = test.items, p.adjust = "BH"))
...
empirical_ES(model_anchor, plot=FALSE, as.table = TRUE) ## Item Level
empirical_ES(model_anchor, DIF=FALSE, plot=FALSE) ## Scale Level

## Item Plots
ES.item <- empirical_ES(model_anchor, plot=TRUE, as.table = TRUE)  
ES.item$main <- ""
ES.item$legend$top$args$key$text[[1]] <- c('Focal - Other', 'Reference - White')
...
ES.test$main <- "" 
ES.test$legend$top$args$key$text[[1]] <- c('Focal - Other', 'Reference - White') 
